...
[TestFixture]
internal sealed class GetTicketViewInfoEndpointTests : ApiTestsBase
{
    ...
    public async Task Get_WithValidTicketId_ShouldReturn200OK()
    {
        ...
        var response = await Client.GetAsync($"{EndpointUrl}/{_ticket.Id.IdValue}/info");
    ...
    public async Task Get_WithInvalidTicketId_ShouldReturn400BadRequest()
    {
        ...
        var response = await Client.GetAsync($"{EndpointUrl}/{Guid.NewGuid()}/info");
    ...
    public async Task Get_WithoutPermissions_ShouldReturn403Forbidden()
    {
        ...
        await PermissionService.RevokePermissionAsync(_customer.Id.IdValue, Permissions.Tickets.View);
